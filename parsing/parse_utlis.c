...
t_node *msh_head_combine(t_data *cur_data ,t_node *left, t_node *right)
{
	...
	head_node = msh_new_node(PIPE_N, 42); // imkn tbadeel type wa9ila
	if (!head_node)
...
t_node *msh_new_node(t_node_type n_type, t_token_types t_type)
{
	...
	if (n_type == PIP_N || n_type == CMD_N)
	{
		...
		red_n_node->type = msh_red_type(type);
		if (!red_n_node->type)
...
int	msh_join_args(char **args, t_data *cur_data)
{
	...
	args = NULL;
	while (cur_data->cur_token && cur_data->cur_tokens->type == WORD_T)
	{
		tmp = *args;
		*args = msh_join_args_sp(cur_token->val, *args, ' ');
		...
		{
			tmp = msh_join_args_sp(cur_data->cur_token->val, *args, " ");
			if (!tmp)
				return (cur_data->err_prs->perr_type = MEMO_E, 0);
			free(*args);
			*args = tmp;
...
bool msh_red_list(t_data *cur_data, t_red_node **red_list)
{
	...
	{
		red_node = msh_new_node(1337, cur_data->cur_tokens->type); //check if i node use this 3 option in her remove it
		if (!red_node)
			return (cur_data->err_prs->perr_type = MEMO_E, false);
		msh_next_token(cur_data);
		if (!cur_data->cur_tokens || cur_data->curr_tokens->type != WORD_T)
